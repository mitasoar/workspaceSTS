...
- 어떤 홈페이지를 만들어볼까 하는 고민중에, 가장 기본이며 중요한 CRUD 기능을 다양하게 구현할 수 있는 '대학교 학사 홈페이지'로 결정하였습니다.
...
- 데이터베이스는 물리적 설계, 논리적 설계 전부 직접 각자 맡은 파트부분을 담당하여 설계하였습니다.
- 그리고 오라클 클라우드를 사용하여 테이블과 데이터를 공유하여 사용하였습니다.
- 형상관리는 SVN을 사용하였고, 기능을 구분지어 개발하였기에 별다른 브랜치 사용없이 깔끔한 형상관리로 진행되었습니다.

---

## 기능 목록

- 공통 및 관리자 기능(회원등록 및 로그인 등 회원 기능, 실시간 채팅 기능, 회원간 메신저 기능, 게시글 및 댓글 등 게시판 기능, 회원 및 게시판 카테고리 관리 등 관리자 기능)
