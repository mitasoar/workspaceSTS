...
public class StudentController {
	...
	//null체크
	@ResponseBody
	@GetMapping("/nullCheckAjax")
	public boolean nullCheckAjax(HttpSession session) {
		if(((MemberVO)session.getAttribute("loginInfo")).getStuNo() != null) {
			return true;
		}
		else {
			return false;
	...
	public String stuInfo(HttpSession session, StudentVO studentVO) {
		//MEM_NO로 STU_NO 조회 -> studentVO에 STU_NO 저장
		studentVO.setStuNo(((MemberVO)session.getAttribute("loginInfo")).getStuNo());
		
		DeptVO deptVO = new DeptVO();
		deptVO.setDeptNo(adminService.selectStudentDetail(studentVO).getDoubleNo());
		
		session.setAttribute("stuInfo", adminService.selectStudentDetail(studentVO));
	...
	public List<DeptAppVO> deptAppAjax(DeptAppVO deptAppVO, StudentVO studentVO) {
		...
		deptAppVO.setApplyDept(studentService.selectDept(deptAppVO).getDeptNo());
	...
	// 내 성적 확인 페이지로 이동
		...
		public String myScore(Model model, HttpSession session, SemesterVO semVO, GradeVO gradeVO) {
			...
			gradeVO.setStuNo(memberVO.getStuNo());
		...
		public List<TimeVO> classSchedule(HttpSession session){
			...
			return studentService.selectClassTime(memberVO.getStuNo());
		}
