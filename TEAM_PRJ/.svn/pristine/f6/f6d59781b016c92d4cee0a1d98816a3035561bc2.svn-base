package com.kh.teamprj.util;

import java.util.ArrayList;
import java.util.Calendar;
import java.util.List;

public class MyDateUtil {
	private static Calendar cal = Calendar.getInstance();
	
	//오늘 날짜를 문자열로 리턴
	public static String getNowDateToString() {
		int year = cal.get(Calendar.YEAR);
		int month = cal.get(Calendar.MONTH) + 1;
		String strMonth =  month / 10 == 0 ? "0" + month : "" + month;
		int date = cal.get(Calendar.DATE);
		String strDate = date / 10 == 0 ? "0" + date : "" + date;
		int hour = cal.get(Calendar.HOUR);
		int minute = cal.get(Calendar.MINUTE);
		String strMinute = minute / 10 == 0 ? "0" + minute : "" + minute;
		int second = cal.get(Calendar.SECOND);
		String strSecond = second / 10 == 0 ? "0" + second : "" + second;
		
		int ampm = cal.get(Calendar.AM_PM); // => 오전/오후
		if(ampm == Calendar.PM) {
			hour += 12;
		}
		
		String strHour = hour / 10 == 0 ? "0" + hour : "" + hour;
		
		return year + "-" + strMonth + "-" + strDate + " "+ strHour + ":" + strMinute + ":" + strSecond;
	}
	
	//최근 5년을 리턴
	public static List<Integer> getNowYear() {
		int year = cal.get(Calendar.YEAR);
		
		List<Integer> list = new ArrayList<Integer>();
		for(int i = year - 4 ; i <= year ; i++) {
			list.add(i);
		}
		
		return list;
	}
	
	public static int[] getSemDate() {
		int nowMonth = Integer.parseInt(getNowDateToString().substring(5 , 7));
		int[] sem = new int[2];
		sem[0] = cal.get(Calendar.YEAR);
		
		
		if(nowMonth <= 6 && nowMonth >= 1) {
			sem[1] = 1;
		}
		else if(nowMonth <= 12 && nowMonth >= 7){
			sem[1] = 2;
		}

		return sem;
	}
	
}
