...
$(document).ready(function() {
	$.ajax({
		...
		success: function(data) {
			let bCode = '<div class="d-inline-block pe-3 ps-1 border-end" role="button" onclick="viewBoard(this);" style="font-weight: bold;">공지사항</div>';
			data.forEach((e, i) => {
...
function viewBoard(div, cateType) {
	...
	if (div != selectCategory) { // 선택한 카테고리가 다른 카테고리일 때
		selectCategory = div; // 선택한 카테고리로 데이터 변경
