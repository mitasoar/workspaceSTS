$(document).ready(function() {
	$.ajax({
		...
		success: function(data) {
			let bCode = '<div class="d-inline-block pe-3 border-end" role="button" onclick="viewBoard();">공지사항</div>';
			data.forEach((e, i) => {
...
function viewBoard(cateType) {
	$.ajax({
		...
		success: function(data) {
			let bCode = '';
			if (data.length == 0) {
				bCode += '<li class="col-10 text-center list-inline">해당 게시판의 게시글이 존재하지 않습니다</li>';
			} else {
				data.forEach((e, i) => {
					bCode += '<li class="row mb-1">';
					bCode += '<div class="col-8 ms-4">';
