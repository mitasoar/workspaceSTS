...
public class MemberController {
	...
	@GetMapping("/list")
	@ResponseBody
	public List<MemberVO> list(String name) { // 회원 목록 ajax
		return memberService.selectMemberList(name);
	}
	
	@GetMapping("/memList")
	public String memList(Model model, MemberVO member, String nowPage) { // 회원관리 페이지 이동
		if (nowPage == null) {
			nowPage = "1";
		}
		
		int total = memberService.countMemberList(member);
		
		member.setPaging(new PagingVO(total, Integer.parseInt(nowPage)));
		
		model.addAttribute("memPage", member);
		model.addAttribute("memList", memberService.selectMemberListByAdmin(member));
	...
	public MemberVO memDetail(String memNo) {
		return memberService.selectMemberOneByAdmin(memNo);
	}
	
	@PostMapping("/withdraw")
	@ResponseBody
	public void withdraw(String[] withdrawList) {
		memberService.updateWithdrawList(withdrawList);
	}
	
	@PostMapping("/rejoin")
	@ResponseBody
	public void rejoin(String[] rejoinList) {
		memberService.updateRejoinList(rejoinList);
	}
	...
	public String login(MemberVO member, HttpSession session){ // 로그인 요청 ajax
		...
		if (loginMember != null && pwEncoder.matches(member.getMemPw(), loginMember.getMemPw())) { // 로그인 성공
