...
public class BoardController {
	...
	public String boardWrite(Model model, String cateNo, HttpSession session) {
		MemberVO member = (MemberVO)session.getAttribute("loginInfo");
		
		model.addAttribute("categoryList", boardService.selectBoardCategoryList(member.getEmpType() == null ? member.getMemType() : member.getEmpType()));
	...
	@ResponseBody
	public String boardWrite(BoardVO board, MultipartFile[] files, HttpSession session) throws IllegalStateException, IOException {
		MemberVO member = (MemberVO)session.getAttribute("loginInfo");
...
//	    }
	...
	public String boardList(Model model, String nowPage, BoardVO board, ReplyVO reply) {
		...
		if (board.getCateNo() != null) {
			board.setCateName(boardService.selectCateName(board.getCateNo()));
		}
	...
	public String editBoard(String boardNo, Model model, HttpSession session) {
	...
	@GetMapping("/cateList")
	@ResponseBody
	public List<BoardCategoryVO> cateList(HttpSession session) {
		MemberVO member = (MemberVO)session.getAttribute("loginInfo");
		
		String memType = null;
		
		if (member != null) {
			memType = member.getEmpType() == null ? member.getMemType() : member.getEmpType();
		}
		
		return boardService.selectBoardCategoryList(memType);
	}
