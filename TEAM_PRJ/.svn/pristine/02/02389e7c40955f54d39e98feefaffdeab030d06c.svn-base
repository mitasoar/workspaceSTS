...
<div class="container">
	...
	<table class="table">
		...
		<tbody align="center" id="tbody">
			...
			<c:forEach items="${academicAppList }" var="app" varStatus="status">
				<tr>
					<td><input type="checkbox" class="form-check-input chk" id="chk${fn:length(academicAppList) - status.index }" value="${app.appNo }" onclick="checkUtil();" 
					...
					<td class="stuNoTd" id="stuNoTd${fn:length(academicAppList) - status.count }">${app.stuNo }</td>
					<td>${app.memberVO.memName }</td>
					<td class="applyTypeTd" id="applyTypeTd${fn:length(academicAppList) - status.count }">${app.applyType }</td>	
					<td>${app.applyDate }</td>
					<c:choose>
	...
	<input type="button" class="btn btn-primary approveBtn" value="일괄승인" onclick="multiApprove();">
</div>
