        ...
        success : function(data){
            ...
            $.each(data, function(key, value){ 
                ...
                if (value.updateDate == null) {
					a += '<div class="d-inline-block col-4 text-end commentInfo_'+value.replyNo+'">'+'작성자 : '+ value.memName + ', 작성일 : ' + value.createDate;
				} else {
					a += '<div class="d-inline-block col-4 text-end commentInfo_'+value.replyNo+'">'+'작성자 : '+ value.memName + ', 작성일 : ' + value.updateDate;
				}
...
function commentUpdate(cno, content, secret){
    ...
    if (secret == 'Y') {
	  	a += '<div class="form-check form-switch"><input class="form-check-input" type="checkbox" role="switch" name="isSecret_' + cno + '" value="Y" checked><label class="form-check-label" for="isSecret">비밀댓글</label></div>';
	} else {
		a += '<div class="form-check form-switch"><input class="form-check-input" type="checkbox" role="switch" name="isSecret_' + cno + '" value="Y"><label class="form-check-label" for="isSecret">비밀댓글</label></div>';
	}
