...
<body>
	<div class="w-75 mx-auto mt-3">
		<c:if test="${empty boardPage.cateNo}">
			<h4 class="text-center mt-2 mb-4">공지사항</h4>
		</c:if>
		<c:if test="${not empty boardPage.cateNo}">
			<h4 class="text-center mt-2">${boardPage.cateName}</h4>
			<div class="text-end">
				<button class="btn btn-outline-dark" type="button" onclick="location.href='/board/boardWrite?cateNo=${boardPage.cateNo}'"<c:if test="${empty loginInfo}">disabled</c:if>>글쓰기</button>
			</div>
		</c:if>
		  ...
		  <ul class="pagination  justify-content-center">
		    ...
		    <c:forEach begin="${boardPage.paging.startPage }" end="${boardPage.paging.endPage }" var="p">
		   		<li class="page-item <c:if test="${p == boardPage.paging.nowPage}">active</c:if>" aria-current="page">
	      			<a class="page-link link-secondary" href="/board/boardList?nowPage=${p}&cateNo=${boardPage.cateNo}&boardTitle=${boardPage.boardTitle}&boardContent=${boardPage.boardContent}&boardWriter=${boardPage.boardWriter}&replyContent=${boardPage.reply.replyContent}&replyWriter=${boardPage.reply.replyWriter}&isNotice=${boardPage.isNotice}">${p}</a>
	    		</li>
			</c:forEach>
		    <c:if test="${boardPage.paging.endPage != boardPage.paging.lastPage}">
			    <li class="page-item">
			      <a class="page-link link-secondary" href="/board/boardList?nowPage=${boardPage.paging.endPage + 1}&cateNo=${boardPage.cateNo}&boardTitle=${boardPage.boardTitle}&boardContent=${boardPage.boardContent}&boardWriter=${boardPage.boardWriter}&replyContent=${boardPage.reply.replyContent}&replyWriter=${boardPage.reply.replyWriter}&isNotice=${boardPage.isNotice}" aria-label="Next">
		...
		<nav class="navbar navbar-light">
		  <div class="col-12">
		    <form class="d-flex ps-5" id="searchForm" action="#" onsubmit="return searchBoard();">
		    	<div class="form-check form-check-inline col-1 m-0 p-0 pt-2 me-2">
