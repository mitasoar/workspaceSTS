...
function callDeptList(coll) {
	$.ajax({ // 선택한 단과대학 학과 리스트 ajax
		url: "/admin/collChangeAjax",
		type: "GET",
		data: {collNo : coll.value},
		success: function(data) { // data : 단과대학 학과 리스트
			let htmlCode = '';
			
			data.forEach((e, i) => {
				htmlCode += '<option value="' + e.deptNo + '">' + e.deptName + '</option>';
			});
			
			document.getElementById('formDept').innerHTML = htmlCode;
		},
	})
}
...
window.onload = function() {
	if (document.getElementById('nonType') != null) { // 회원 등록 시 지정된 경로로 진입하지 않은 경우
		...
		location.href = '/main/main'; // 다시 메인화면으로 이동
	}

	if (document.querySelector('#formColl') != null) {
		callDeptList(document.querySelector('#formColl'));
	}
}