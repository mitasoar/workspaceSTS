						...
						<td>
							...
							<c:choose>
								<c:when test="${lec.maxNum == lec.nowNum }">
									<button type="button" class="btn btn-secondary" disabled="disabled">신청 마감</button>
								</c:when>
								<c:when test="${not empty regLecList }">
									<c:forEach items="${regLecList }" var="regLec">
										<c:if test="${regLec.lecNo eq lec.lecNo }">
											<button type="button" class="btn btn-white" style="pointer-events: none; font-weight: bold;">신청 완료</button>
											<!-- <b>신청 완료</b> -->
											<c:set var="result" value="false"/>
										</c:if>
									</c:forEach>
									<c:if test="${result }">
										<button type="button" class="btn btn-primary" onclick="regClass('${lec.lecNo}');">수강 신청</button>
									</c:if>
								</c:when>
								<c:otherwise>
									<button type="button" class="btn btn-primary" onclick="regClass('${lec.lecNo}');">수강 신청</button>
